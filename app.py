...
def graph(v1, v2, v_cross):
    ...
    fig.add_trace(go.Scatter3d(x=[BOUND, 0, 0], y=[0, BOUND, 0], z=[0, 0, BOUND],mode='text', text=['X Axis', 'Y Axis', 'Z Axis'], textposition='top center',textfont=dict(size=14, color='black'), showlegend=False ))
    def add_vector(fig,vector,color,name):
        fig.add_trace(go.Scatter3d(x=[0, vector[0]], y=[0, vector[1]], z=[0, vector[2]],mode='lines', line=dict(color=color, width=6), name=name))
        mag=np.linalg.norm(vector)
        if mag>0:
            u_norm,v_norm,w_norm=vector[0]/mag,vector[1]/mag,vector[2]/mag      
            cone_size=BOUND*0.08
            fig.add_trace(go.Cone(x=[vector[0]],y=[vector[1]],z=[vector[2]],u=[u_norm],v=[v_norm],w=[w_norm],sizemode="absolute", sizeref=cone_size,anchor="tip",colorscale=[[0, color],[1, color]],showscale=False,name=name,showlegend=False))

    add_vector(fig,v1,'red','Vector A')
    add_vector(fig,v2,'blue','Vector B')
    add_vector(fig,v_cross,'green','A×B')

    axis_config=dict(range=[-BOUND,BOUND],showbackground=False,showgrid=True,gridcolor='lightgray',zeroline=False,showticklabels=True,title="")
    fig.update_layout( template="plotly_white",scene=dict(xaxis=axis_config,yaxis=axis_config,zaxis=axis_config,aspectmode='cube'), margin=dict(l=0, r=0, b=0, t=0),height=600)
    return fig
